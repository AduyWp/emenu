  ...
  <div class="q-gutter-y-md q-mt-md">
    <q-card class="no-shadow">
      <q-tabs
        ...
        narrow-indicator
      >
        <q-tab class="text-grey1 text-capitalize" name="listVoucher" :label="$t('voucher.listVoucher')" />
        <q-tab class="text-grey1 text-capitalize" name="myVoucher" :label="$t('voucher.myVoucher')" />
      </q-tabs>
      <q-separator />
      <q-tab-panels v-model="tab" animated>
        <q-tab-panel name="listVoucher" class="bg-grey-extralight">
        ...
        <q-tab-panel name="myVoucher">
          <div class="q-mt-sm q-px-none">
            <span v-if="listMyVoucher !== []">
              <div class="row" v-for="voucher in listMyVoucher" :key="voucher" @click.prevent="goToProductDetail(voucher.id)">
                <div class="col-12 col-sm-12 col-md-12 q-px-sm q-mb-md">
                  <div
                    v-ripple:blue
                    class="relative-position text-weight-bold bg-white text-black card-menu text-center"
...
// @click.prevent="onSubmit(voucher.Item.Variants[0].id, voucher.Item.id)"
export default defineComponent({
  setup () {
    const router = useRouter()
    const $q = useQuasar()
    const tab = ref('listVoucher')
    const myPoint = ref()
      ...
      await authRepo.tadaOtp(accessToken.value, phone.value, cardno.value).then((tadaOtp) => {
        try{
          if (cardno.value !== null) {
            ...
            router.push({path: '/emenu/voucher/redeem/validate', query: { variant: selectedVariant, itemId: itemId, walletName: walletName.value, cardPin: cardPin.value, pointVoucher: pointVoucherRedeem.value }})
          }
        } catch (err) { 
          console.log(err)
          $q.loading.hide()
